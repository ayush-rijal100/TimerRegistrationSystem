...
import type { HarnessContext } from "./contextBuilder.js";
import type { HarnessIntent, HarnessPlan } from "./planner.js";

interface AiPlannerSuccess {
  source: "ai";
  plan: HarnessPlan;
  confidence: number;
  rawIntent: string;
...
interface AiPlannerSkipped {
  source: "skipped";
  reason: string;
}

interface AiPlannerFailed {
  source: "failed";
  reason: string;
}
...
const allowedIntents: HarnessIntent[] = [
  "WHO_AM_I",
  "SHOW_CONTEXT",
  "GET_MY_PROJECTS",
  "GET_ADMIN_PROJECTS",
...
function isHarnessIntent(value: string): value is HarnessIntent {
  return allowedIntents.includes(value as HarnessIntent);
}
...
function buildUserPrompt(userMessage: string, context: HarnessContext): string {
  return [
...
function parseAiPlan(rawText: string): AiPlannerSuccess {
  ...
  const confidenceValue = Number((parsed as { confidence?: unknown }).confidence ?? 0);
...
export async function planRequestWithAi(
  userMessage: string,
