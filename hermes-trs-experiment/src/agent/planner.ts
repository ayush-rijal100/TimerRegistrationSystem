...
export function planRequest(userMessage: string): HarnessPlan {
  ...
  if (
    normalized.startsWith("remember recommendation feedback:") ||
  ...
  if (
    normalized.includes("show recommendation memory") ||
  ...
  if (
    normalized.includes("show registered tools") ||
  ...
  if (
    normalized.includes("show context") ||
  ...
  if (
    normalized.includes("show all projects") ||
  ...
  if (
    normalized.includes("show all users") ||
  ...
  if (
    normalized.includes("recommend actions") ||
  ...
  if (
    normalized.includes("manager insight") ||
  ...
  if (
    normalized.includes("missing entries") ||
  ...
  if (
    normalized.includes("show project assignments") ||
  ...
  if (
    normalized.includes("what can admin") ||
...
export function applyPlannerCorrections(
  userMessage: string,
  plan: HarnessPlan
): HarnessPlan {
  const normalized = userMessage.toLowerCase().trim();

  const asksForRegisteredTools =
    normalized.includes("show registered tools") ||
    normalized.includes("show tool registry") ||
    normalized.includes("what tools can you use") ||
    normalized.includes("what tools are registered") ||
  ...
  const asksForSpecificMemory =
    normalized.includes("correction memory") ||
    normalized.includes("show corrections") ||
  ...
  const asksForExplicitMissingLogs =
    normalized.includes("missing entries") ||
    normalized.includes("missing time entries") ||
  ...
  const asksForBroadManagerDiagnosis =
    normalized.includes("who seems behind") ||
  ...
  if (
    plan.intent === "GET_MISSING_ENTRIES_REPORT" &&
