...
export interface ToolVerificationSuccess {
  ok: true;
}

export interface ToolVerificationFailure {
  ok: false;
  toolName: string;
  reason: string;
  itemIndex?: number;
...
export type ToolVerificationResult =
  | ToolVerificationSuccess
  | ToolVerificationFailure;

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null;
}

function verifyArray(value: unknown, toolName: string): ToolVerificationResult {
...
function firstFailure(
  checks: ToolVerificationResult[]
): ToolVerificationResult {
  return checks.find((check) => !check.ok) ?? { ok: true };
}

export function verifyAdminProjectsResponse(
  value: unknown
): ToolVerificationResult {
  ...
  const projects = value as ProjectResponse[];
...
export function verifyAdminUsersResponse(
  value: unknown
): ToolVerificationResult {
  ...
  const users = value as UserResponse[];
...
export function verifyUtilizationReportResponse(
  value: unknown
): ToolVerificationResult {
  ...
  const report = value as UtilizationReportResponse[];
...
export function verifyMissingEntriesReportResponse(
  value: unknown
): ToolVerificationResult {
  ...
  const report = value as MissingEntriesReportResponse[];
  ...
  for (let index = 0; index < report.length; index++) {
    ...
    const invalidDateIndex = missingDates.findIndex(
